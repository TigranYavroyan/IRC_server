...
std::string Replies::err_needModeParm(cref_str channelname, cref_str mode) {
	return ":" + server_name + " 696 #" + channelname + " * You must specify a parameter for the key mode. " + mode + crlf();
}

std::string Replies::err_invaliDModeParm(cref_str channelname, cref_str mode) {
	return ":" + server_name + " 696 #" + channelname + " Invalid mode parameter. " + mode + crlf();
}

std::string Replies::err_keySet(cref_str channelname) {
	return ":" + server_name + " 467 #" + channelname + " Channel key already set. " + crlf();
}

std::string Replies::err_unknownMode(cref_str nickname, cref_str channelname, cref_str mode) {
	return ":" + server_name + " 472 " + nickname + " #" + channelname + " " + mode + " :is not a recognised channel mode" + crlf();
}
...
std::string Replies::err_noSuchNick(cref_str channelname, cref_str name) {
	return ":" + server_name + " 401 #" + channelname + " " + name + " :No such nick/channel" + crlf();
}

std::string Replies::err_incorpass(cref_str nickname) {
	return ":" + server_name + " 464 " + nickname + " :Password incorrect !" + crlf();
}

std::string Replies::err_alreadyRegistered(cref_str nickname) {
	return ":" + server_name + " 462 " + nickname + " :You may not reregister !" + crlf();
}

std::string Replies::err_noNickName(cref_str nickname) {
	return ":" + server_name + " 431 " + nickname + " :No nickname given" + crlf();
}

std::string Replies::err_nickInUse(cref_str nickname) {
	return ":" + server_name + " 433 " + nickname + " :Nickname is already in use" + crlf();
}

std::string Replies::err_oneUsNick(cref_str nickname) {
	return ":" + server_name + " 432 " + nickname + " :Erroneus nickname" + crlf();
}

std::string Replies::err_notRegistered(cref_str nickname) {
	return ":" + server_name + " 451 " + nickname + " :You have not registered!" + crlf();
}

std::string Replies::err_cmdnotFound(cref_str nickname, cref_str command) {
	return ":" + server_name + " 421 " + nickname + " " + command + " :Unknown command" + crlf();
}

std::string Replies::err_cannotJoin(cref_str nickname, cref_str channelname) {
	return ":" + server_name + " 475 " + nickname + " " + channelname + " :Cannot join channel (+k or +i or full)" + crlf();
}